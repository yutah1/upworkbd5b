  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-indigo-600 text-white p-4 sticky top-0 z-40 shadow-md flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold">মাসিক বেতন</h1>
      </div>
      ...
      <div className="p-4 max-w-2xl mx-auto space-y-4 mt-4">
        {salaries.length === 0 ? (
          <div className="text-center p-8 bg-white rounded-xl shadow-sm border border-gray-200 text-gray-500">
            এখনও কোনো মাসিক বেতন পোস্ট করা হয়নি।
          </div>
        ) : (
          salaries.map(salary => (
            <div key={salary.id} className="bg-white p-5 rounded-xl shadow-sm border border-indigo-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <button className="bg-indigo-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-indigo-700 transition whitespace-nowrap">
                ক্লেম করুন
              </button>
            </div>
          ))
